...
class TestParseCommand:
    def test_anchor_db(self):
        ...
        for col in enums:
            assert hasattr(db_anchor_enums, change_all_nonletters_to_underscores(
                col)), f'No enum for {col} in our code'

            n_option_in_col_ours = len(
                getattr(db_anchor_enums, change_all_nonletters_to_underscores(col)))
            n_option_in_col_remote = len(enums[col])
            assert n_option_in_col_ours == n_option_in_col_remote, \
                f"""Number of options in enum {col} differ: {n_option_in_col_ours} \
!= {n_option_in_col_remote}.\n {getattr(db_anchor_enums,
                                        change_all_nonletters_to_underscores(col))}\n{enums[col]}"""
